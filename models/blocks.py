...
def get_block_ops(block_name, radius, in_dim, out_dim, layer_ind, args):
    if block_name == 'unary':
        return UnaryBlock(in_dim, out_dim, args.use_batch_norm, args.batch_norm_momentum)
        ...
        return SimpleBlock(block_name, in_dim, out_dim, radius, layer_ind, args)
        ...
        return ResnetBottleneckBlock(block_name, in_dim, out_dim, radius, layer_ind, args)
...
class UnaryBlock(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        :param out_dim:  输出维度
        :param use_bn:   是否使用BN
        :param bn_momentum: BN冲量
        :param no_relu:  不使用激活函数
        """
...
class BatchNormBlock(nn.Module):
    def __init__(self, in_dim, use_bn, bn_momentum):
        """
        :param in_dim: 输入维度
        :param use_bn: 是否使用BN, 如果不使用BN的话，使用bias代替
        :param bn_momentum: bn冲量
        """
    ...
    def forward(self, x):
        if self.use_bn:
            ...
            x = x.transpose(0, 2)  # 变换维度
            x = self.batch_norm(x)  # 进行BN处理
            x = x.transpose(0, 2)  # 将维度变换回来
            return x.squeeze()  # 去掉增加的维度
...
class SimpleBlock(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, args):
        """

        :param block_name: 模块名称
        :param in_dim: 输入维度
        :param out_dim: 输出维度
        :param radius: 半径
        :param layer_ind: 层的编号
        :param args: 参数列表
        """
        super(SimpleBlock, self).__init__()
        self.bn_momentum = args.batch_norm_momentum
        self.use_bn = args.use_batch_norm
        self.layer_ind = layer_ind
        self.block_name = block_name
        self.in_dim = in_dim
        self.out_dim = out_dim

        current_extent = radius * args.KP_extent / args.conv_radius

        self.KPConv = KPConv(args.num_kernel_points,
                             args.in_points_dim,
                             in_dim,
                             out_dim // 2,
                             current_extent,
                             radius,
                             fixed_kernel_points=args.fixed_kernel_points,
                             KP_influence=args.KP_influence,
    ...
    def forward(self, x, batch):
        ...
        return self.leaky_rule(self.batch_norm(x))
...
def gather(x, idx, method=2):
    """
    自定义的gather函数，按照idx从data中提取数据
    :param x: [n1,d]
    :param idx:[n2,max_num]
    :param method:
    :return:
    """
    if method == 0:
