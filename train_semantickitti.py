...
from datasets.semantickitti_loader import *
import models
...
train_set = SemanticKittiDataset(args, train=True, balance_classes=True)
val_set = SemanticKittiDataset(args, train=False, balance_classed=False)

train_sampler = SemanticKittiSampler(train_set)
val_sampler = SemanticKittiSampler(val_set)
...
val_loader = DataLoader(val_set,
                        ...
                        num_workers=args.workers,
                        pin_memory=True)
train_sampler.calib_max_in(args, train_loader, verbose=True)
val_sampler.calib_max_in(args, val_loader, verbose=True)
...
randla_net = torch.nn.DataParallel(kpconv_net)

print("4. Setting Optimization Solver")
if args.optimizer == 'Adam':
    optimizer = torch.optim.Adam(kpconv_net.parameters(), lr=args.lr, betas=(args.momentum, args.beta),
                                 weight_decay=args.weight_decay)
elif args.optimizer == 'SGD':
    optimizer = torch.optim.SGD(kpconv_net.parameters(), lr=args.lr, momentum=args.momentum,
